...
export const updateProfile = async (req: Request, res: Response) => {
    ...
    const { phone, university, avatar } = req.body;
...
export const getAmbassadorStats = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const rankIndex = leaderboard.findIndex(item => item._id.toString() === userId.toString());
        const globalRank = rankIndex !== -1 ? rankIndex + 1 : leaderboard.length + 1;
        ...
        console.log(`Window: ${startOfWeek.toISOString()} to ${endOfWeek.toISOString()}`);
        ...
        currentWeekTasks.forEach(t => console.log(`- Task: ${t.title}, Due: ${t.dueDate.toISOString()}, Bonus: ${t.isBonus}`));
