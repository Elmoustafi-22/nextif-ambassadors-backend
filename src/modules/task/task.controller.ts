...
export const getMyTasks = async (req: Request, res: Response) => {
    ...
    // 3. Merge data
    console.log(`Getting tasks for user ${req.user.id}. Total found: ${tasks.length}`);
    tasks.forEach(t => console.log(`- Task: ${t.title}, Due: ${t.dueDate.toISOString()}, isBonus: ${t.isBonus}`));
...
export const submitTask = async (req: Request, res: Response) => {
    ...
    const proofFiles = files ? files.map(f => f.path) : req.body.proofFiles || [];
    ...
    console.log(`Submitting task ${id} for user ${req.user.id}, setting status to ${status}`);
