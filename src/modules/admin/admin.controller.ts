...
export const sendAnnouncement = async (req: Request, res: Response) => {
    ...
    res.status(201).json({ message: `Announcement sent to ${ambassadorIds.length} ambassadors` });
};
...
export const createAmbassador = async (req: Request, res: Response) => {
    ...
    if (existing) {
        return res.status(400).json({ message: "Ambassador with this email already exists" });
    }
...
export const bulkOnboardAmbassadors = async (req: Request, res: Response) => {
    ...
    try {
        ...
        for (const record of records) {
            ...
            if (!record.email || !record.firstName || !record.lastName || !record.university) {
                errors.push({ record, message: "Missing required fields (firstName, lastName, email, university)" });
                continue;
            }

            const existing = await Ambassador.findOne({ email: record.email.toLowerCase() });
            if (existing) {
...
export const forceResetAmbassadorPassword = async (req: Request, res: Response) => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized"});
    ...
    res.json({ message: "Ambassador password reset triggered. They must reset on next login.", data: ambassador });
};
...
export const getDashboardStats = async (req: Request, res: Response) => {
    ...
    try {
        const [activeAmbassadors, pendingSubmissions, openComplaints, totalNotifications] = await Promise.all([
        ...
        res.json({
            ...
            recentActivity: recentActivity.map(item => ({
                id: item._id,
                ambassadorName: item.ambassadorId ? `${(item.ambassadorId as any).firstName} ${(item.ambassadorId as any).lastName}` : 'Unknown',
